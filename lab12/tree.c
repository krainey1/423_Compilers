...
/* Map a type_name to a background fill color for dot nodes */
static const char *type_color(type_name t) {
  switch (t) {
    case int_type:      return "#cce5ff"; /* blue   */
    case long_type:     return "#cce5ff";
    case double_type:   return "#d4edda"; /* green  */
    case float_type:    return "#d4edda";
    case bool_type:     return "#fff3cd"; /* yellow */
    case char_type:     return "#fde8d8"; /* orange */
    case string_type:   return "#f8d7da"; /* red    */
    case array_type:    return "#e2d9f3"; /* purple */
    case function_type: return "#d1ecf1"; /* teal   */
    case null_type:     return "#e2e3e5"; /* gray   */
    case unit_type:     return "#e2e3e5";
    default:            return "white";
  }
}

static void print_branch(tree_t *t, FILE *f) {
  char *label = pretty_print_name(t);
  const char *tstr = name(t->typetag);  /* from symtab.c */
  const char *col  = type_color(t->typetag);
  fprintf(f, "N%d [shape=box style=filled fillcolor=\"%s\" label=\"%s\\ntype: %s\"];\n",
          t->id, col, label, tstr);
  free(label);
}
...
static void print_leaf(tree_t *t, FILE *f) {
  ...
  char *esc = dot_escape(t->leaf->lexeme ? t->leaf->lexeme : "");
  const char *tstr = name(t->typetag);
  const char *col  = type_color(t->typetag);
  fprintf(f, "N%d [shape=box style=\"dotted,filled\" fillcolor=\"%s\" label=\" %s \\n ",
          t->id, col, catname);
  fprintf(f, "text = %s \\l lineno = %d \\l type = %s \\l\"];\n",
          esc, t->leaf->line, tstr);
  free(esc);
}
