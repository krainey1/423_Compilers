...
/* Walk the tree and stamp each node's typetag with its inferred type */
static void stamp_types_r(struct tree *t, SymbolTable global)
{
    if (!t) return;
    /* recurse children first */
    for (int i = 0; i < t->nkids; i++)
        stamp_types_r(t->kids[i], global);
    t->typetag = infer_type(t, global);
}

void stamp_types(struct tree *root, SymbolTable global)
{
    stamp_types_r(root, global);
}
